...
def get_read_only_token(namespace: str = None, service_account_name: str = None) -> JSONResponse:
    """
    ...
    try:
        ...
        logging.error(f"Error creating token for service account {service_account_name} in namespace {namespace}: {e}")
        return JSONResponse(
            status_code=500,
            content={"error": str(e)}
        )
    
def create_token_for_sa(namespace: str, sa_name: str, expiration_seconds=DEFAULT_EXPIRATION_SECONDS) -> str:
    """
    ...
    token_response = core_v1.create_namespaced_service_account_token(
        name=sa_name,
        namespace=namespace,
        body={"apiVersion": "authentication.k8s.io/v1", "kind": "TokenRequest", "spec": token_spec.to_dict()}
    )
